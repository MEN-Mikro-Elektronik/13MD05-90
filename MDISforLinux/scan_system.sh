...
check_if_men_cpu () {
    ...
    if [ "${i}" == "${detectedCpu}" ]; then
        G_cpu=$i;
        debug_print echo "found matching CPU: ${i}"
        break
    else
        G_cpu="";
    fi
    ...
    for adrs in ${ID_EEPROM_ADRESSES}; do
        for dev in /dev/i2c-*; do
            ...
            if [[ ${SCAN_SIM} == 1 && ${smbus} == ${SCAN_SIM_SMBUS} ]]; then
                G_cpu=${SCAN_SIM_CPU}
                debug_print echo "- simulating CPU ${G_cpu} on bus ${smbus}"
                break
            fi
...
fill_entry_dsc_smb_scan_list () {
    debug_print echo "fill smb devices scan list into system.dsc file"
    local dscTmpFile=$(<${OUTPUT_DIR_PATH}/${DSC_FILE})
    local formattedSmbDeviceList="$(echo "${G_smbDeviceList}" | sed ':a;N;$!ba;s/\n/\\n/g' )"
...
add_device_smb_scan_list () {
    debug_print echo "add_device_smb_scan_list"
    local tmpSmbDeviceList=$(cat ${1}/smb_device.tpl | sed "s/SCAN_SMB_DEV_NUMBER/${2}/g;s/SCAN_SMB_DEV_NAME/${3}/g")
...
scan_for_pci_devs () {
    ...
    #  25   0   5  15   0   0x12d8   0xe110    0x0000
        ...
        if [ "$pcivend" == "0x1172" ] && [ "$pcidevid" == "0x203d" ] && [ "$pcisubvend" == "0x00b9" ]; then
            ...
            G_cPciRackSlotSerial=`expr $G_cPciRackSlotSerial + 1`
            create_entry_dsc_d203_a24 $DSC_TPL_DIR $count_instance_d203_a24 $pcibusslot $pcibus "G204_A24" $pcidevnr ${OUTPUT_DIR_PATH}/${DSC_FILE}
        fi
        ...
        debug_print echo "Found d203 no. $count_instance_d203"
...
scan_for_mmodules () {
    ...
    local mm_pci_bus=`printf "%x" ${4}`
    local mm_pci_dev=`printf "%x" ${5}`
    local mm_pci_fun=`printf "%x" ${6}`
    local bar_address=`lspci -s ${mm_pci_bus}:${mm_pci_dev}.${mm_pci_fun} -v | grep 'Memory at' | head -n 1 | awk '{print $3}'`
    if [ "${7}" == "A08/D16" ]; then
...
get_board_outermost_position () {
    ...
    local DeviceNr=`grep "${1} * ${2} * ${3}" ${TMP_PCIDEVS} | wc -l`
...
if [ "$main_cpu" == "SC24" ]; then
    map_sc24_fpga
    cat $DSC_TPL_DIR/sc24.tpl | sed "s/SCAN_WIZ_MODEL/$wiz_model_cpu/g; s/SCAN_SMBNR1/`printf \"0x%x\" $((G_SmBusNumber - 1))`/g; s/SCAN_SMBNR2/`printf \"0x%x\" $G_SmBusNumber`/g;" >> ${OUTPUT_DIR_PATH}/${DSC_FILE}
    blacklist_warning_message
...
# create SC25 based Bx70x CPU model - no FPGA mapping necessary here
    ...
    cat $DSC_TPL_DIR/sc25.tpl | sed "s/SCAN_WIZ_MODEL/$wiz_model_cpu/g;" >> ${OUTPUT_DIR_PATH}/${DSC_FILE}
    blacklist_warning_message
    cat $DSC_TPL_DIR/Makefile.sc24.tpl >> $MAKE_FILE
elif  [ "$main_cpu" == "SC31" ]; then 
    cat $DSC_TPL_DIR/sc31.tpl | sed "s/SCAN_WIZ_MODEL/$wiz_model_cpu/g; s/SCAN_SMB_BUS_NR/`printf \"0x%x\" $G_SmBusNumber`/g;" >> ${OUTPUT_DIR_PATH}/${DSC_FILE}
    blacklist_warning_message
