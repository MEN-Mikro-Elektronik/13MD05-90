 ...
 * R: 1) Cosmetics.
 * M: 1) Some register debug prints were swapped at little endian machines.
 *
 * Revision 1.6  2010/08/24 14:00:58  rt
 * R: 1) Not compilable.
 * M: 1) Renamed wrong macro name.
 *
 * Revision 1.5  2010/06/29 13:19:52  rt
 * R: 1) Not compilable for kernel version > 2.6.26.
 * M: 1) Support new location of semaphore.h.
 *
 * Revision 1.4  2009/07/09 11:53:58  rt
 * R: 1.) Access to slave window register space is not working anymore if
 *        another slave window is requested.
 *    2.) Make driver more save against race conditions.
 *    3.a) 8/16 bit RMW cycles not working.
 *      b) RMW: Values read are swapped.
 *    4.) Large zero-copy DMAs (>408k) read/write wrong data.
 *    5.) VME system reset not working at A19/A20.
 *    6.) Not all memory resources are freed if module is unloaded.
 * M: 1.) Added inUse parameter to VME4L_RESRC.
 *    2.) Some small changes to spin locks (disable IRQs more often...).
 *    3.a) Shift RMW mask depending on access width.
 *      b) Swap values read.
 *    4.) Changed vmeAddr parameter in DmaSetup to pointer to vmeAddr.
 *    5.) A19/A20 do not reset themselfes if VME reset is activated, so we
 *        have to release VME reset after >200 ms.
 *    6.) Rename vmeOut to vmeIn for freeing inbound windows resources.
 *
 * Revision 1.3  2009/06/03 19:06:25  rt
 * R: 1.) Make timing/arbitration adjustable.
 *    2.) Inbound window for TSI148 register access missing.
 *    3.) No DMA support.
 *    4.) Incomplete bus error support.
 *    5.) No Requester mode support.
 *    6.) No RMW support.
 *    7.) No mailbox support.
 *    8.) No location monitor support.
 *    9.) Address violation if an open inbound window is set-up with same size
 *        but different VME addr.
 * M: 1.) TSI148_VMCTRL, TSI148_VCTRL, TSI148_BLT_DCTL, TSI148_VMEFL
 *        and TSI148_PCIXCAP added.
 *    2.) TSI148_SLVX... defines added.
 *    3.) Tsi148_DmaSetup, Tsi148_DmaStart, Tsi148_DmaStop, Tsi148_DmaStatus
 *        implemented.
 *    4.) Tsi148_BusErrGet implemented.
 *    5.) Tsi148_RequesterModeGet and Tsi148_RequesterModeSet implemented.
 *    6.) Tsi148_RmwCycle implemented.
 *    7.) Tsi148_MboxRead and Tsi148_MboxWrite implemented.
 *    8.) Tsi148_LocMonRegRead and Tsi148_LocMonRegWrite implemented.
 *    9.) Use old physical address in this case.
 *
 * Revision 1.2  2009/04/30 21:41:46  rt
 * R: 1) Reworked.
 * M: 1a) Inbound windows.
 *     b) Outbound windows.
 *     c) A16/A24/A32/A64 PIO VME accesses.
 *     d) Interrupts.
 *     e) Support for SMP.
 *     f) System controll function.
 *
