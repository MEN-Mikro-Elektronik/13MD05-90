 ...
 *
 * $Log: bb_smb2.c,v $
 * Revision 1.21  2014/07/22 13:15:39  ts
 * R: block data access added in bzr repo
 * M: rechecked in driver to keep mcvs in sync
 *
 * Revision 1.20  2012/05/21 17:40:41  ts
 * R: compile failed for kernel 3.1
 * M: removed include of i2c-id.h, has been removed and integrated to i2c.h
 *
 * Revision 1.19  2011/05/18 17:23:23  CRuff
 * R: not buildable with kernel 2.6.18
 * M: only use .name and .owner in i2c_driver up to kernel 2.6.15
 *
 * Revision 1.18  2011/02/17 15:29:47  CRuff
 * R: 1. support for kernel versions up to 2.6.37
 * M: 1. use DEFINE_SEMAPHORE instead of DECLARE_MUTEX from kernel 2.6.36
 *
 * Revision 1.17  2010/10/25 11:17:04  CSchuster
 * R: definition of DECLARE_MUTEX not found for kernel 2.6.35
 * M: include semaphore.h for kernels > 2.6.35
 *
 * Revision 1.16  2010/10/19 10:23:04  CRuff
 * R: compiler error for kernel 2.6.33
 * M: fix linux version check: use new detect signature with kernel 2.6.33 also
 *
 * Revision 1.15  2010/07/02 14:35:58  CRuff
 * R: support of kernel 2.6.34
 * M: 1. include header linux/slab.h for kmalloc, kfree
 *    2. changed signature of oss_smb2_detect (remove int kind)
 *    3. remove struct addr_data and hand over the addressed directly in
 *       i2c_driver via .address_list
 *
 * Revision 1.14  2010/05/18 15:12:12  CRuff
 * R: 1. MDIS descriptor contains common SMB device addresses (not shifted)
 *    2. Porting to MDIS5 (according to porting guide rev. 0.9
 * M: 1. in function getClientFromAddrAndBusnr, shift the client address
 *       before using it (addr >> 1);
 *       ATTENTION: old smb device descriptors have to be changed!!
 *    2. added support for 64bit (API)
 *
 * Revision 1.13  2010/04/14 13:47:58  CRuff
 * R: 1. i2c api change in linux kernel 2.6.31
 *    2. debug level was not influenced by MDIS descriptor debug level
 * M: 1a.changed member types according to i2c upgrading clients documentation
 *    1b.implemented methods oss_smb2_probe, oss_smb2_remove and oss_smb2_detect
 *       and updated i2c_driver structure
 *    2. read key DEBUG_LEVEL from descriptor in SMB2BB_Init() and use it
 *
 * Revision 1.12  2009/09/30 14:58:16  ts
 * R: BBIS kernel checks SetIrqHandle() in BBIS drivers
 * M: return 0, not ILL_FUNC so BBIS init works
 *
 * Revision 1.11  2009/09/08 18:42:58  CRuff
 * R: i2_smbus_write_quick() not supported any more
 * M: return error if used
 *
 * Revision 1.10  2008/04/30 12:03:05  ts
 * added missing member i2c_driver->driver->name, caused Oops in Kernel > 2.6.19
 *
 * Revision 1.9  2008/04/25 15:20:40  ts
 * - API change: adapted to workqueue API change during 2.6.20
 * - bugfix: changed Parameter of Exit function to pointer of smb handle
 * - corrected Parameter description in doxygen headers
 *
 * Revision 1.8  2007/03/28 16:41:26  ts
 * Made Workqueue timeout call a printk to be displayed in any case
 *
 * Revision 1.7  2006/12/15 16:19:04  ts
 * replaced BBIS fkt13 with setIrqHandle
 *
 * Revision 1.6  2006/12/15 14:22:16  ts
 * workqueue added for SMB accesses in IRQ contexts
 *
 * Revision 1.5  2006/08/09 09:33:24  ts
 * handle different SMBus numbers
 *
 * Revision 1.4  2006/08/07 17:21:31  ts
 * unified i2c_driver struct
 *
 * Revision 1.3  2006/08/07 12:13:47  ts
 * SMBus access moved here from oss_smb.c
 *
 * Revision 1.2  2006/06/21 17:19:28  ts
 * Cosmetics, Warnings removed, Headers changed
 *
 * Revision 1.1  2006/06/20 18:25:43  ts
 * Initial Revision
 *
 *---------------------------------------------------------------------------
