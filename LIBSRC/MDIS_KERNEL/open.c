 ...
 *
 * $Log: open.c,v $
 * Revision 1.21  2013/10/24 10:00:18  ts
 * R: gcc 4.6 under Ubuntu 12.10 warned about type mismatch of mk_dbglevel
 * M: use DESC_GetUint32 on u_int32 temp variable then pass to mk_dbglevel
 *
 * Revision 1.20  2011/05/23 11:49:35  CRuff
 * R: 1. support for mixed mem/io chameleon tables
 *    2. support for pci domains
 *    3. avoid irq status changes in low level device while changing bbis irq
 *       status
 * M: 1. in GetBoardParams(), try to get new BBIS_CFGINFO_ADDRSPACE instead of
 *       old BBIS_BRDINFO_ADDRSPACE; if not found, fall back to old info code
 *    2a) in GetBoardParams(), get BBIS_CFGINFO_PCI_DOMAIN from BBIS
 *       (returns error if not supported by BBIS)
 *    2b) cosmetics: extended debug prints to also show the pci domain
 *    2c) for all OSS calls that use the pci bus number, call with merged
 *        bus/domain
 *    2d) in PciGetAddrReg(): use pciDomain for pci_find_bus calls
 *    3a) call MK_LOCK / UNLOCK in MDIS_EnableIrq and handle locking correctly
 *        in other methods
 *    3b) removed OSS_IrqMaskR / OSS_IrqRestore from MDIS_EnableIrq and
 *        BrdIrqEnable()
 *
 * Revision 1.19  2011/02/17 14:45:05  CRuff
 * R: support of chameleon mixed mode (mem / io )
 * M: in MDIS_GetBoardParams(), ask for BBIS_CFGINFO_ADDRSPACE; try traditional
 *    BBIS_BRDINFO_ADDRSPACE only if first call returns error
 *
 * Revision 1.18  2010/04/14 10:08:44  CRuff
 * R: pci_find_slot is removed from kernel 2.6.31
 * M: replace pci_find_slot by pci_get_slot; thereby, set pci domain to 0 for now
 *
 * Revision 1.17  2009/08/04 17:04:25  CRuff
 * R: make 64bit compatible
 * M: 1. use type U_INT32_OR_64 for value parameter in getStat call in
 *       MDIS_InitialOpen()
 *    2. MapAdressSpaces(): cast virt. address to U_INT32_OR_64 before casting
 *       to void*
 *
 * Revision 1.16  2009/06/09 17:48:53  ts
 * R: BBIS handlers require setIrqHandle() function now
 * M: added call to BBIS SetIrqHandle()
 *
 * Revision 1.15  2007/12/10 13:11:20  ts
 * use new Irq handler Flags IRQF_xxx instead SA_SHIRQ for new 2.6 kernels
 *
 * Revision 1.14  2007/08/09 19:24:33  ts
 * Re-added EM03 Family Irq fix
 *
 * Revision 1.13  2007/03/28 17:08:38  ts
 * Cosmtics, removed Adding of EXT0 value for EM3 Family here
 *
 * Revision 1.12  2006/11/10 13:42:22  ts
 * Bugfix:
 * MDIS_FinalClose was called after request_module() didnt find the drivermodule
 *
 * Revision 1.11  2006/08/17 13:48:54  ts
 * EM3 Family: FPGA Unit IRQ+=MPC85xx_IRQ_EXT0 (special Interrupt wiring)
 *
 * Revision 1.10  2005/01/19 11:28:45  ts
 * fixed problem with IRQ enabling (added irqEnableKey to MK_DEV),cosmetic
 *
 * Revision 1.9  2004/07/16 15:06:09  kp
 * cosmetics
 *
 * Revision 1.8  2004/06/09 11:12:34  kp
 * Linux 2.6 support
 * Changed PCI access functions to use kernel routines (ACPI aware)
 *
 * Revision 1.7  2003/06/06 09:09:37  kp
 * Bug fix in installing RTAI interrupts
 *
 * Revision 1.6  2003/04/11 16:26:24  kp
 * finalized RTAI port
 *
 * Revision 1.5  2003/02/21 13:34:43  kp
 * added support for RTAI and non-mdis drivers
 *
 * Revision 1.4  2003/02/21 10:12:03  kp
 * fixed problem when mapping PCI I/O spaces
 *
 * Revision 1.3  2001/09/18 15:18:48  kp
 * Problem workaround: allow two LL devices to use the same slot if they
 * are onboard slots (F2 GPIO/LED)
 *
 * Revision 1.2  2001/08/16 10:15:55  kp
 * support VME devices
 *
 * Revision 1.1  2001/01/19 14:58:37  kp
 * Initial Revision
 *
 *---------------------------------------------------------------------------
...
int32 MapAddressSpaces(MK_DEV *dev, DESC_HANDLE *devDescHdl)
{
	...
	{	
		...
		if( dev->space[n].type == OSS_ADDRSPACE_MEM )
		{
			...
			request_region( (unsigned long) dev->space[n].physAddr,
							dev->space[n].reqSize,
							dev->devName );
		}
...
int32 MDIS_InstallSysirq(MK_DEV *dev)
{
...
#endif /* CONFIG_MEN_VME_KERNELIF */
	{
		...
		if((error = request_irq( dev->irqVector, MDIS_IRQFUNC,
								 ...
								 dev->devName,
								 dev )) < 0 ){
			DBGWRT_ERR((DBH," *** InstallSysirq: can't install interrupt "
						"(linux error=%d)\n", -error ));
			return(ERR_MK_IRQ_INSTALL);
		}
	}
